    ...
    internal class Gears
    {
        ...
        public static void ShowGears(int showType)
        {
            ...
            for (int i = 0; i < gearCount; i++)
            {
                ...
                Console.WriteLine($"-{(showType == 1 ? "" : i + 1)} {Program.gearDb[i].Name}  | {eff}+{Program.gearDb[i].Effect}  | {Program.gearDb[i].Desc}  | {(Program.character.GearsIHave[i] ? "이미 구매한 상품입니다" : Program.gearDb[i].Price)}");
